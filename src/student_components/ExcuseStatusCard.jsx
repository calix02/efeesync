...
function ExcuseStatusCard({data, view,edit}){
    const color = 
    data.status === "Approved" ? 'bg-[#E5FDE4] text-[#15A310]'
    :data.status === "Pending" ? 'bg-[#FFF8E4E5] text-[#D09D00]'
    :data.status === "Rejected" ? 'bg-[#F8D1D2] text-[#DE0004]' 
    : 'bg-white'
    return(
             <div className="w-[100%] py-4 px-6 bg-white shadow-[2px_2px_3px_grey] border-1 border-[#621668] rounded-lg">
            <div>
                <div className={ `w-25 h-7 rounded-2xl ${color} shadow-[2px_2px_1px_grey] flex justify-center items-center text-sm font-semibold`}>{data.status}</div>
            </div>
            <div className="mt-2 text-sm flex flex-col">
                <span className="text-lg font-semibold">{data.event_name}</span>
                <span className="mt-3">Absence Date: {data.absent_date}  </span>
