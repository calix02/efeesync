...
function ExcuseStatusCard({data, view,edit, formatDateStr, del}){
    ...
    return(
            ...
            <div className="mt-2 text-sm flex flex-col">
                ...
                <span className="mt-3">Absence Date: {formatDateStr(data?.event_attend_date)}  </span>
                <span>Submitted: {formatDateStr(data.attendance_excuse_submitted_at)} </span>
            </div>
            <div className="flex justify-center gap-2 mt-4  text-sm">
                ...
                <button onClick={() =>edit(data)}
                    ...
                    className="w-20 h-6 disabled:bg-[#d6d6d6] disabled:cursor-default disabled:scale-100 disabled:text-[#7d7d7d] shadow-[1px_1px_1px_black] border-[#f0f0f0] transition duration-300 hover:scale-105 hover:text-white hover:bg-[#2d345b] border-1 cursor-pointer rounded-3xl bg-[#E6EDFD] text-[#2F6FDD]">
                    Edit
                 </button>
