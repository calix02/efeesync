...
function Monetary({monetarySanction, code, animate, formatDateStr}){
    ...
    return(
        <div className={` ${color} ${animate}  w-[100%] h-120 overflow-y-scroll font-poppins font-semibold hide-scrollbar border-2 shadow-[2px_2px_3px_grey] bg-white rounded-lg  px-6`}>
            <div className="border-b-4  text-xl pt-3 py-1  font-semibold">
                <span>Monetary</span>
            </div>
            {monetarySanction.length > 0 ? 
            (monetarySanction.map((ms)=>(
                <div className="w-[100%] bg-[#fcfcfc] hover:scale-103 transition duration-300  min-h-25 py-2 flex items-center border border-[#e0e0e0] shadow-[2px_2px_2px_gray] px-4 rounded-lg  mt-3">
