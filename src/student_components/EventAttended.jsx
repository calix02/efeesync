import "../animate.css";
function EventAttended(){
    const animate = "left-In";
    return(
        <div className={` ${animate} w-145 h-135 border-2 shadow-[2px_2px_3px_grey] bg-white rounded-lg border-[#15A310] text-[#15A310] px-6`}>
            <div className="border-b-4 border-[#15A310] text-xl pt-2 py-1  font-semibold">
                <span>Event Attended</span>
            </div>
            <div className="w-[100%] h-15 flex items-center border-2 px-4 rounded-lg border-[#15A310] mt-3">
                <div className="w-[100%] h-12 border-l-4 border-[#15A310] px-6 flex flex-col">
                    <span className="text-xl font-semibold">Year-End Party</span>
                    <span className="text-sm text-[#252525]">Date: April 30, 2025</span>
                </div>

            </div>
            <div className="w-[100%] h-15 flex items-center border-2 px-4 rounded-lg border-[#15A310] mt-3">
                <div className="w-[100%] h-12 border-l-4 border-[#15A310] px-6 flex flex-col">
                    <span className="text-xl font-semibold">Year-End Party</span>
                    <span className="text-sm text-[#252525]">Date: April 30, 2025</span>
                </div>

            </div>
            <div className="w-[100%] h-15 flex items-center border-2 px-4 rounded-lg border-[#15A310] mt-3">
                <div className="w-[100%] h-12 border-l-4 border-[#15A310] px-6 flex flex-col">
                    <span className="text-xl font-semibold">Year-End Party</span>
                    <span className="text-sm text-[#252525]">Date: April 30, 2025</span>
                </div>

            </div>

        </div>
    );
}
export default EventAttended;