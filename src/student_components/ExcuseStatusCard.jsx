...
function ExcuseStatusCard({status,view,edit}){
    ...
    : 'bg-white'
   const excuseData = Array.from({length: 3}, (_,i) =>({
        eventName : `Tribute to Senior`,
        absentDate: `May 22, 2025`,
        submitDate: `May 22, 2025`,
   }));
    return(
        <>
        {excuseData.map((d, id) =>(
            ...
            <div className="mt-2 text-sm flex flex-col">
                <span className="text-lg font-semibold">{d.eventName}</span>
                <span className="mt-3">Absence Date: {d.absentDate}  </span>
                <span>Submitted: {d.submitDate} </span>
            </div>
            <div className="flex justify-center gap-2 mt-4  text-sm">
                <button onClick={() =>view(d)} className="w-20 h-6 border-1 shadow-[1px_1px_1px_black] border-[#f0f0f0] hover:scale-105 transition duration-300 hover:bg-[#346132] hover:text-white   cursor-pointer rounded-3xl bg-[#DCF5E0] text-[#2CA74D]">View</button>
