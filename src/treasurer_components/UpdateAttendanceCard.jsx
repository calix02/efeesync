...
const UpdateAttendanceCard = React.forwardRef(({animate, onAnimationEnd,onClose, data}, ref) =>{
    const [studentName, setStudentName] = useState(data?.name);
    const [yearSection, setYearSection] = useState(data?.yearSection);

    const [morningIn, setMorningIn] = useState(data?.morningIn);
    const [morningOut, setMorningOut] = useState(data?.morningOut);
    const [afternoonIn, setAfternoonIn] = useState(data?.afternoonIn);
    const [afternoonOut, setAfternoonOut] = useState(data?.afternoonOut);
    ...
    React.useEffect(()=>{
        if(data){
            ...
            setMorningIn(data.morningIn);
            setMorningOut(data.morningOut);
            setAfternoonIn(data.afternoonIn);
            setAfternoonOut(data.afternoonOut);
        }
    },[data]);
    ...
    return( 
        ...
        onAnimationEnd={onAnimationEnd}>
            ...
            <div className="mt-6 border-b-4 border-[#8A2791]">
                <span className="text-[#8A2791] font-semibold lg:text-xl text-lg">Update Attendee</span>
            </div>
            <form onSubmit={(e)=>{
                submit();
                onClose();
            ...
            <div className="mt-4">
                ...
                <select onChange={changeAfternoonOut} value={afternoonOut} name="" id="" className=" px-2 border-2 cursor border-[#8A2791] h-8 rounded-md w-[100%] mb-4">
                    <option value=""></option>
                    <option value="Present">Present</option>
                    <option value="Absent">Absent</option>
                    <option value="Excuse">Excuse</option>
                ...
                <button type="submit"  className="bg-[#561b5a] w-[100%] cursor-pointer rounded-md text-white h-8">Update Attendee</button>
            </form>
        </div>
       
    );
});
export default UpdateAttendanceCard;